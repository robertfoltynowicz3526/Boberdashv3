...
function initializeApp() {
    ...
    // ⬇⬇⬇ DODANE: wstrzyknięcie pola Notatka do istniejącego modala „Zakończ Zlecenie”
    // (index.html zostaje bez zmian)
    (function ensureZakonczenieNotatkaField() {
        if (!completeModalForm) return;
        const ALREADY = completeModalForm.querySelector('#zakonczenie-notatka');
        if (ALREADY) return; // już dodane

        const group = document.createElement('div');
        group.className = 'form-group';
        group.innerHTML = `
            <label for="zakonczenie-notatka">Notatka (opcjonalnie):</label>
            <textarea id="zakonczenie-notatka" rows="3" placeholder="Opis wykonanych prac, uwagi serwisowe..."></textarea>
        `;
        // wstawiamy notatkę nad listą magazynu (przed <hr> już obecnym)
        const hr = completeModalForm.querySelector('hr');
        if (hr && hr.parentNode) {
            hr.parentNode.insertBefore(group, hr);
        } else {
            // awaryjnie na koniec formularza
            completeModalForm.appendChild(group);
        }
    })();
    // ⬆⬆⬆ KONIEC: wstrzykiwanie pola Notatka
    ...
    async function pokazHistorieSerwisowaMaszyny(maszynaId, maszynaNazwa) {
        ...
        try {
            ...
            // Delegacja zdarzeń dla modala historii
    ...
    function wyswietlZlecenia() {
        ...
        przefiltrowaneZlecenia.forEach(zlecenie => {
            ...
            if (zlecenie.status === 'aktywne' || zlecenie.status === 'nieprzypisane') {
                ...
                ukonczoneHtml += `<li data-id="${zlecenie.id}"><span><strong>${nazwaMaszyny}</strong> (Nr: ${zlecenie.nrZlecenia})<br><em>Ukończono (${zlecenie.dataUkonczenia||'b.d.'})</em><br>Fakturowano: <strong>${zlecenie.wyfakturowaneGodziny||0}h</strong> | Typ: <strong>${zlecenie.typZlecenia||'?'}</strong>${uzyteCzesciHtml}${notatkaHtml}</span><div><button class="btn-details details-zlecenie-btn">Szczegóły</button><button class="btn-edit edit-zlecenie-btn">Edytuj</button><button class="btn-edit reopen-btn">Otwórz ponownie</button><button class="delete-btn">Usuń</button></div></li>`;
            }
        });
    ...
    async function obslugaListyZlecen(event) {
        ...
        if (event.target.classList.contains('complete-btn')) {
            ...
            if (docSnap.exists()) {
                ...
                // wyczyść pole notatki (dodane w Części 1)
                const noteEl = document.getElementById('zakonczenie-notatka');
        ...
        // 🔁 Otwieranie ponownie zakończonego zlecenia
        if (event.target.classList.contains('reopen-btn')) {
            if (!confirm("Czy na pewno chcesz ponownie otworzyć to zlecenie?")) return;
            try {
                const zRef = doc(db, "zlecenia", docId);
                const snap = await getDoc(zRef);
                if (!snap.exists()) return alert("Nie znaleziono zlecenia.");
                const data = snap.data();
                const nowaHistoria = [...(data.historia || []), {
                    timestamp: new Date().toISOString(),
                    akcja: "Ponownie otwarto zlecenie."
                }];
                await updateDoc(zRef, { status: 'aktywne', historia: nowaHistoria });
                alert("Zlecenie ponownie otwarte.");
            } catch (e) {
    ...
    // ✅ TU DODANA NOTATKA: odczytujemy wartość z #zakonczenie-notatka i zapisujemy jako zakonczenieNotatka (opcjonalna)
    async function obslugaZakonczeniaZlecenia(event) {
        event.preventDefault();
        const docId = document.getElementById('complete-zlecenie-id').value;
        const notatkaPole = document.getElementById('zakonczenie-notatka');
        const notatka = notatkaPole ? (notatkaPole.value || '').trim() : '';
        const dane = { 
            status: 'ukończone', 
    ...
    document.getElementById('export-zlecenia-btn').addEventListener('click', () => {
        ...
        const dane = _wszystkieZleceniaCache
            ...
            .map(({id, createdAt, status, uzyteCzesci, historia, ...reszta}) => ({
                ...
                notatka: reszta.zakonczenieNotatka || ''
            }));
    ...
    // Zamknięcia modalów po kliknięciu w tło
    ...
    wyswietlPrzejazdy();    // (pusta, zostawiona dla spójności)
    wyswietlMagazyn();
